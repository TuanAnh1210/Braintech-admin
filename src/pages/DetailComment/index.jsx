...
const DetailComment = () => {
    ...
    const columns = [
        ...
        { title: 'Avatar', dataIndex: 'avatar', key: 'avatar', render: () => (<Image width={'20%'} src="https://picsum.photos/200"/>) },
        { title: 'Time', dataIndex: 'time', key: 'time', render: () => (<p>20/1/2024</p>)},
        { title: '', dataIndex: 'action', key: 'action', render: (_,{id}) => (<Button danger onClick={()=>confirm('Sure?')&&onHandleDelete(id)}>Delete</Button>)},
    ];
    const newData = data.filter((item)=>item.id == id)
    const onHandleDelete = (id) => {
        fetch(`https://jsonplaceholder.typicode.com/comments/${id}`,{
            method: 'DELETE'
        }).then(()=>{navi(-1),alert('Remove successfully!')})
    }
    return (
